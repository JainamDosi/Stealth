    ...
    return (
        <section className="relative py-12 md:py-20 px-0 bg-[#FFFDF9] overflow-hidden">
            ...
            <div className="space-y-4 md:space-y-6 faded-edges pb-6">
                <MarqueeRow items={TESTIMONIALS_ROW_1} direction="normal" />
                <MarqueeRow items={TESTIMONIALS_ROW_2} direction="reverse" />
            </div>
        </section>
    );
}
...
function MarqueeRow({ items, direction }: { items: typeof TESTIMONIALS_ROW_1, direction: "normal" | "reverse" }) {
    const animationClass = direction === "normal" ? "animate-marquee" : "animate-marquee-reverse";
    return (
        <div className="flex whitespace-nowrap overflow-hidden">
            <div className={`flex ${animationClass} shrink-0`}>
                {items.map((item, i) => <TestimonialCard key={`${direction}-1-${i}`} item={item} />)}
            </div>
            <div className={`flex ${animationClass} shrink-0`}>
                {items.map((item, i) => <TestimonialCard key={`${direction}-2-${i}`} item={item} />)}
            </div>
        </div>
    );
}
