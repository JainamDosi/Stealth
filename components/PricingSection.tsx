...
export default function PricingSection() {
    return (
        <section className="relative py-20 md:py-32 px-6 md:px-8 bg-gradient-to-b from-white to-[#F9F7F2]/30 overflow-hidden">
                    ...
                    {pricingPlans.map((plan, index) => (
                        <div
                            ...
                            className={`relative flex flex-col ${plan.bgColor} rounded-[2.5rem] p-8 md:p-10 shadow-lg transition-all duration-500 reveal-delay-${index + 1} ${plan.featured
                                ? 'md:scale-105 z-10 border-2 border-white/20'
                                : 'border border-primary-brown/5'
                                } group`}
                        >
                            {/* Watermark restricted to a hidden-overflow container inside the card */}
                            {plan.featured && (
                                <div className="absolute inset-0 overflow-hidden rounded-[2.5rem] pointer-events-none">
                                    <div className="absolute -top-10 -right-10 w-64 h-64 opacity-[0.15] rotate-[20deg] scale-125">
                            ...
                            {plan.featured && (
                                <div className="absolute -top-4 left-1/2 -translate-x-1/2 bg-primary-brown text-white px-6 py-2 rounded-full text-[9px] font-black tracking-widest uppercase z-30 shadow-md">
                                    MOST POPULAR
                                </div>
                            )}

                            <div className="space-y-6 md:space-y-8 relative z-10 flex-1 flex flex-col">
                                <div className={`w-12 h-12 md:w-16 md:h-16 rounded-2xl ${plan.featured ? 'bg-white/20' : 'bg-orange-50'} flex items-center justify-center text-3xl transition-transform duration-500`}>
                                    {plan.emoji}
                                </div>
